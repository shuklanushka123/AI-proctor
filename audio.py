...
SUS_FREQUENCY_FREQUENCY = 2               # calculates SUS *n* times every sec
SOUND_AMPLITUDE_THRESHOLD = 10          # amplitude considered for SUS calc 

# packing *n* frames to calculate SUS
FRAMES_COUNT = int(CALLBACKS_PER_SECOND/SUS_FREQUENCY_FREQUENCY)
...
def print_sound(indata, outdata, frames, time, status):
    ...
    if count == FRAMES_COUNT:
        ...
        if avg_amp > SOUND_AMPLITUDE_THRESHOLD:
            SUS_COUNT += 1
            if SUS_COUNT == 1:
                print("Cheating Warning Level 1")
            elif SUS_COUNT >= 2:
                print("Cheating Warning Level 2 :- Final Warning!!")
